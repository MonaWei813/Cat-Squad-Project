//The file main to control the apply the job, and it will get the information job_token, and user_token
//save them to localstorgae.
...
.then((object) => {
    ...
    .then((object) => {
        ...
        for(var i = 1; object[i] != undefined; i++){
            if(object[i].job_token == job_token){
                console.log("enter in applied");
                result = "applied";
            }
        }
        if(result == "none"){
            ...
            apply_btn.addEventListener("click",function(){
                confirm("You have applied this job. Please Choose another one.");
            });
        }
