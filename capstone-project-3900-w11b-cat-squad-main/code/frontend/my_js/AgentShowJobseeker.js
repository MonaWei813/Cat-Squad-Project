// The file is main to show all of jobseeker and control the search button to Agent.
...
api.showresumedetail(resume_token,user_token)
.then((response) => {
...
.then((object) => {
    ...
    document.getElementById("p5").innerHTML = `<b>Email Address: </b> ${object.email_address}`+'&nbsp;&nbsp;&nbsp;<a href= ${object.email_address}` style="padding-right: 20px; font-size: 36px;" id = "contact" >&#x1f4e7;</a>';
    ...
    api.checksavedresume(resume_token,user_token)
    .then((response) => {
        // Case 1: Successfully Login
        if (response.status === 200){
            return response.object;
    ...
    save.addEventListener("click",function(){
        if(save.style.color == "darkorange"){
            var v = confirm("Do you want to remove this jobseeker from saved jobseeker?");
            if(v == true){
                save.style.color = "#ccc";
                api.deletesavedresume(resume_token,user_token)
                .then((response) => {
                    // Case 1: Successfully Login
                    if (response.status === 200){
                        return response.object;
            ...
            save.style.color = "darkorange";
            api.addsavedresume(resume_token,user_token)
            .then((response) => {
                // Case 1: Successfully Login
                if (response.status === 200){
                    return response.object;
            ...
            confirm("Jobseeker resume saved!");
