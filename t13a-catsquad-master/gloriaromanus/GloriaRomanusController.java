...
public class GloriaRomanusController{
  ...
  private void initialize() throws JsonParseException, JsonMappingException, IOException, InterruptedException {
    ...
    // select in loading screen)//这是要加loading的
    ...
    String []menus = {"invasion_menu.fxml","Goal1.fxml","invasion_menu2.fxml","Goal2.fxml"};
    ...
    stackPaneMain.setAlignment(controllerParentPairs.get(0).getValue(), Pos.CENTER_RIGHT);
    stackPaneMain.setAlignment(controllerParentPairs.get(1).getValue(), Pos.CENTER_LEFT);
      ...
      if (f.getGeometry() instanceof org.geojson.Point) {
        ...
        switch (faction) {
          case "Gaul":
            ...
            s = new PictureMarkerSymbol(new Image((new File("images/New Project2.png")).toURI().toString()));
            break;
          case "Rome":
            // you can also pass in a javafx Image to create a PictureMarkerSymbol (different to BufferedImage)
            s = new PictureMarkerSymbol("images/New Project (2).png");
            break;
  ...
  public void handleendturn() throws JsonParseException, JsonMappingException, IOException {
    ...
    stackPaneMain.setAlignment(controllerParentPairs.get(2).getValue(), Pos.CENTER_LEFT);
    stackPaneMain.setAlignment(controllerParentPairs.get(3).getValue(), Pos.CENTER_RIGHT);
    WhetherGoal(goal1Controller.s1.Player1(), goal1Controller.s1.Player2());

  }
  public void handleendturn2() throws JsonParseException, JsonMappingException, IOException {

    goal1Controller.startturn += 1;
    ...
    stackPaneMain.setAlignment(controllerParentPairs.get(0).getValue(), Pos.CENTER_RIGHT);
    stackPaneMain.setAlignment(controllerParentPairs.get(1).getValue(), Pos.CENTER_LEFT);
    System.out.println("find the turn:"+ goal1Controller.startturn);
    checktraining(goal1Controller.s1.Player1(),goal1Controller.startturn);
    checktraining(goal1Controller.s1.Player2(),goal1Controller.startturn);
    checkRoad(goal1Controller.s1.Player1(),goal1Controller.startturn);
    checkRoad(goal1Controller.s1.Player2(),goal1Controller.startturn);

    WhetherGoal(goal1Controller.s1.Player1(), goal1Controller.s1.Player2());
    
    Player p1 = goal1Controller.s1.Player1();
    Player p2 = goal1Controller.s1.Player2();
    int temp = p1.getTreasury() + p1.getProvinces().size()*20*goal1Controller.startturn;
    p1.settreasury(temp);
    int temp1 = p2.getTreasury() + p2.getProvinces().size()*20*goal1Controller.startturn;
    p2.settreasury(temp1);
  }
  ...
  public void checkRoad(Player player, int turn){
    for(int i = 0; i < player.getProvinces().size(); i++){
      player.getProvinces().get(i).RemoveOnRoad(turn);
    }
  }
  public void WhetherGoal(Player player1, Player player2) throws IOException {
    if(player1.getProvinces().isEmpty()){
      ...
      if(player2.getTreasury() >= 100000 &&  totalwealth >= 400000){
        ...
        FXMLLoader loader = new FXMLLoader(getClass().getResource("Victory2.fxml"));
